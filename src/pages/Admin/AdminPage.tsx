...
import { Shield, Users, Video, ArrowLeft, CheckCircle } from 'lucide-react';
import { useNavigate } from 'react-router-dom';

export const AdminPage: React.FC = () => {
  const navigate = useNavigate();
  const [view, setView] = useState<'menu' | 'embaixadora' | 'tambores'>('menu');
  ...
  const inscricoesFiltradas = inscricoes.filter(i => filtroModalidade === 'Todos' || i.modalidade === filtroModalidade);
  const embaixadorasCount = inscricoes.filter(i => i.modalidade === 'Embaixadora').length;
  const madrinhasCount = inscricoes.filter(i => i.modalidade === 'Madrinha').length;
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white font-sans flex flex-col">
      ...
      <main className="flex-1 p-6 lg:p-10 max-w-7xl mx-auto w-full flex flex-col gap-8 animate-in slide-in-from-bottom-8 fade-in duration-700">
        ...
        {view === 'menu' && (
          <div className="z-10 w-full max-w-5xl mt-8 flex flex-col gap-12 animate-in fade-in duration-1000 mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl mx-auto px-4 mt-6">
              {/* Card 1: Embaixadora */}
              <div 
                onClick={() => setView('embaixadora')}
        ...
        {view === 'embaixadora' && (
          <>
            <div className="flex items-center gap-4 animate-in fade-in slide-in-from-left-4">
              ...
              <h2 className="text-2xl font-black text-white uppercase tracking-widest">Métricas: Embaixadoras</h2>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 flex flex-col gap-2 relative overflow-hidden group shadow-lg hover:shadow-xl transition-shadow">
            ...
            <h2 className="text-4xl font-black text-white z-10">{inscricoes.length}</h2>
          </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 flex flex-col gap-2 relative overflow-hidden group shadow-[0_0_15px_rgba(255,215,0,0.05)] hover:shadow-[0_0_20px_rgba(255,215,0,0.1)] transition-shadow">
            ...
            <h2 className="text-4xl font-black text-white z-10">{embaixadorasCount}</h2>
          </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 flex flex-col gap-2 relative overflow-hidden group shadow-[0_0_15px_rgba(6,182,212,0.05)] hover:shadow-[0_0_20px_rgba(6,182,212,0.1)] transition-shadow">
            ...
            <h2 className="text-4xl font-black text-white z-10">{madrinhasCount}</h2>
          </div>
        </div>
                  ...
                  {inscricoesFiltradas.map((inscricao) => (
                    <tr key={inscricao.id} className="hover:bg-zinc-800/30 transition-colors group">
                      ...
                      <td className="p-4 text-zinc-400 text-sm">
                        {new Date(inscricao.data_nascimento).toLocaleDateString('pt-BR')}
                      </td>
                      <td className="p-4 text-zinc-400 text-sm">
